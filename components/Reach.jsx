...
const INDIA_TOPO_JSON = "/india_accurate.topo.json";
...
const nodes = [
  ...
  },
  {
    ...
    coordinates: [93.9485, 24.8170], /* Manipur region coordinates */
    details: "Impacting Arunachal Pradesh, Mizoram, and Manipur through AI education focused on underserved communities.",
    bullets: [
...
export default function Reach() {
  ...
  const [scale, setScale] = useState(1100);

  useEffect(() => {
    const updateSize = () => {
      const maxW = 800;
      const w = Math.min(window.innerWidth * 0.95, maxW);
        ...
        <div className={styles.mapContainer}>
          ...
          >
            <Geographies geography={INDIA_TOPO_JSON}>
              {({ geographies }) => {
                // If it's TopoJSON, we need to extract the features.
                // React-simple-maps handles raw TopoJSON if the 'geography' prop points to a URL,
                // but if we want to be explicit or if the file structure is complex, we handle it here.
